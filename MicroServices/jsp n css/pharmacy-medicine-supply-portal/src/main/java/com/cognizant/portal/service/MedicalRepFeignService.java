...
public class MedicalRepFeignService {
	 ...
	 */
	public ResponseEntity<?> getRepSchedule(String token, Date dateOfMeeting) {
		log.debug("token{}:", token);
		ResponseEntity<?> response = repFeignClient.getRepSchedule(getTokenWithHeader(token), dateOfMeeting);
		return response;
	}
